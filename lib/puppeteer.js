...
class Puppeteer {
    ...
    async _startSession(client) {
        ...
        if (session.browser) {
            this._stopSession(client);
        }

        this._debug(client, 'starting new session');

        const browser = session.browser = new Browser('https://web.whatsapp.com/');
        await browser.launch();

        client.puppetId = await browser.injectJS('./dist/bootstrap.js', 'WhatsTrapp.launchPuppet();');
    }
    ...
    async _stopSession(client) {
        ...
        if (session.browser) {
            this._debug(client, 'stopping the session');

            await session.browser.stop();

            for (const key in Object.getOwnPropertyNames(session)) {
                delete session[key];
            }
        }
    }
